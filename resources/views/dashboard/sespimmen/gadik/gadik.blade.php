                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-6 text-right text-white">
                            <div class="d-inline-block">
                                ...
                                <a class="btn btn-primary" href="{{url('tambah_gadik_sespimmen')}}"><span class="fa fa-plus"></span> tambah</a>

                                <a class="btn btn-info" id="detail_gadik"><span class="fa fa-eye"></span> detail</a>

                                <a class="btn btn-danger" id="hapus_gadik"><span class="fa fa-trash"></span> hapus</a>
                                @else
                                <a class="btn btn-info" id="detail_gadik"><span class="fa fa-eye"></span> detail</a>
                                @endif
                            </div>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$d->jenis_gadik}}</td>
                                @if(auth()->user()->level == "admin")
                                <td><a class="btn btn-danger text-white" href="{{url('edit_gadik_sespimmen')}}/{{$d->id}}"><span class="fa fa-pencil"></span> edit</a></td>
                                @endif
                            </tr>
                            @endforeach
                        </tbody>
...
<script type="text/javascript">
    ...
    $('input[type="checkbox"]').click(function() {
        if ($(this).prop("checked") == true) {
            id_gadik.push($(this).val());
        } else if ($(this).prop("checked") == false) {
            for (var i = 0; i < id_gadik.length; i++) {

                if (id_gadik[i] === $(this).val()) {
    ...
    let url_hapus = "{{url('hapus_gadik_sespimmen')}}";
    let url_detail = "{{url('detail_gadik_sespimmen')}}";
    let url_img = "{{asset('admin/sespimmen/foto_gadik')}}";
    let html = ``;
    $('#detail_gadik').click(() => {
        $.ajax({
            url: url_detail,
                ...
                $.each(response, function(i, val) {
                    html += `<div class="card mb-3">
                                <form action="" class="p-3" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <label for=""><b>Jenis Gadik</b></label>
                                        <p>${val.jenis_gadik}</p>
                                    </div>
                                    <div class="form-group">
